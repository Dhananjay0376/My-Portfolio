        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-4 right-4 bottom-4 w-[calc(100%-2rem)] md:w-[450px] glass-card z-[201] flex flex-col rounded-[2.5rem] border-border shadow-2xl overflow-hidden"
            >
              ...
              <div className="p-8 border-b border-border relative bg-card/40">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      ...
                      <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-400 border-4 border-background animate-pulse" />
                    </div>
                    <div>
                      <h3 className="text-xl font-black tracking-tighter text-foreground uppercase leading-none">Solution<br/>Architect</h3>
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} className="rounded-xl hover:bg-primary/10 w-12 h-12">
                    <X className="w-6 h-6" />
                  </Button>
                </div>
                ...
                <div className="flex gap-4">
                  <div className="flex-1 flex items-center gap-2 px-3 py-1.5 rounded-lg bg-card/60 border border-border font-mono text-[9px] uppercase tracking-wider text-muted-foreground">
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-hide">
                {messages.length === 0 && (
                  <div className="space-y-8 mt-10">
                    <div className="space-y-4">
                      ...
                      <h2 className="text-3xl font-black tracking-tighter text-foreground leading-[1.1]">HOW CAN WE<br/>ENGINEER TODAY?</h2>
                      <p className="text-muted-foreground text-sm font-light leading-relaxed max-w-[280px]">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {SUGGESTED_QUESTIONS.map((q) => (
                        <button
                          ...
                          className="text-left px-5 py-4 rounded-2xl bg-card/60 border border-border hover:border-primary/40 hover:bg-primary/5 transition-all duration-500 group relative overflow-hidden text-foreground"
                        >
                          ...
                          <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">{q}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {messages.map((m, idx) => (
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      m.role === 'user' 
                        ? 'bg-primary text-primary-foreground rounded-tr-none shadow-[0_10px_30px_rgba(74,127,181,0.2)] font-medium' 
                        : 'bg-card/80 border border-border rounded-tl-none text-foreground'
                    )}>
                      {m.role !== 'user' && (
                ...
                {isLoading && messages.length > 0 && messages[messages.length - 1].role === 'user' && (
                  <div className="flex flex-col gap-2 items-start">
                    <div className="flex items-center gap-3 px-1">
                      <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-muted-foreground">Processing</span>
                      <motion.div 
                    ...
                    <div className="p-5 rounded-[1.5rem] bg-card/80 border border-border rounded-tl-none flex gap-1.5 items-center">
                      {[0, 1, 2].map((i) => (
                        <motion.div
                          key={i}
              ...
              <div className="p-8 bg-card/40 border-t border-border">
                <form onSubmit={handleSubmit} className="flex items-center gap-3 relative">
                  ...
                  <Input
                    ...
                    className="bg-card/80 border-border rounded-2xl pl-12 pr-14 focus-visible:ring-primary h-14 font-mono text-xs tracking-wider text-foreground"
                  />
                  <Button 
                    type="submit" 
                    size="icon" 
                    disabled={isLoading || !inputValue.trim()}
                    className="absolute right-2 w-10 h-10 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground shadow-[0_0_20px_rgba(74,127,181,0.3)] transition-all duration-500 disabled:opacity-20"
                  >
                    <Send className="w-4 h-4" />
                  </Button>
                </form>
                <div className="mt-4 flex justify-between items-center px-2">
                  <span className="text-[8px] font-mono text-muted-foreground uppercase tracking-[0.3em]">Encrypted_Stream_Active</span>
